...
\abstract{We present a mechanism in which the baryon asymmetry and dark
matter share a single origin fixed by \emph{pre-inflationary initial
...
III / SEDE cosmology, Zenodo DOI to be assigned{]}), \textbf{dawn} (this
paper), and \textbf{galactic} (a MOND readout; Paper VIII, not yet
drafted). The scale sector {[}Paper VI / scale sector, Zenodo DOI to be
assigned{]} sets the mass scales that this paper's dark-matter horns
draw on. The full unification is formalized in the umbrella paper
{[}Paper V / USC framework{]}, and every paper cites the frozen
pre-registration falsifier matrix (Zenodo
\texttt{10.5281/zenodo.21415326}).
...
(\texttt{CROSS\_CHECK\_MEMO.md}: ``AGREE''; \(m_X = 1.33\) vs \(1.3255\)
...
\(0.708\) (\texttt{SPONTANEOUS\_CP\_REPORT}). The symmetric-point value
...
(\texttt{CLOSURES\_assumed\_vs\_calculated.md}, N4). We flag the honest
...
operator-by-operator (\texttt{AUDIT\_assumed\_vs\_calculated.md}).
...
(\texttt{CONVENTIONS\_AND\_CORRECTIONS.md}). We adopt
...
(\texttt{CONSOLIDATED\_THEORY.md}) --- a preliminary narrowing (single
...
(\texttt{GLUEBALL\_DM\_assessment.md}): the SIMP ``miracle'' here is a
...
\texttt{predict\_eta\_B.py}} that already encodes the \(\eta_B\) fit;
...
\textasciitilde{}\(10^{9}\) (\texttt{USC\_m3\_derivation.md}). In
...
rule (\texttt{EXTERNAL\_CALC\_SPECS.md}, items S-1/S-2/S-3).
...
(\texttt{PIN16\_assignment\_constructed.md},
\texttt{S3\_GAUGING\_REPORT}). This is \emph{stronger than
...
(Zenodo \texttt{10.5281/zenodo.21415326}).
...
\url{https://github.com/spsingularity/eccg-matter-genesis}. Key scripts
and reports:
...
\item
  \textbf{Sphaleron / \(f_B\):}
  \texttt{CLOSURES\_assumed\_vs\_calculated.md} (N4); Harvey--Turner
  counting.
\item
  \textbf{Spontaneous CP / \(\Delta_{\rm CP}\):}
  \texttt{spontaneous\_cp.py} → \texttt{SPONTANEOUS\_CP\_REPORT.md}
  (scan, \texttt{spontaneous\_cp\_scan.csv}, 40,000 draws).
\item
  \textbf{Pre-inflation reheating cure (D-1):}
  \texttt{sims/preinflation\_reheat\_domain\_check.py} →
  \texttt{D1\_result\_preinflation\_reheating.md}.
\item
  \textbf{Candidate C neutrino inversion (D-2):}
  \texttt{sims/candidateC\_neutrino\_inversion.py},
  \texttt{candidateC\_yield.py} →
  \texttt{D2\_result\_candidateC\_neutrino.md}; figure
  \texttt{figures/candidateC\_neutrino\_inversion.png}.
\item
  \textbf{Wall velocity (S-3):} \texttt{wall\_velocity\_precise.py} →
  \texttt{WALL\_VELOCITY\_PRECISE\_REPORT.md}.
\item
  \textbf{Global scan / benchmark point:}
  \texttt{GLOBAL\_SCAN\_REPORT.md}.
\item
  \textbf{Flavor / \(S_3\) gauging / \(\mathrm{Pin}^+\)
  \(\nu \bmod 16\):} \texttt{FLAVOR\_MODEL\_REPORT.md},
  \texttt{S3\_GAUGING\_REPORT.md},
  \texttt{PIN16\_assignment\_constructed.md}.
\item
  \textbf{Glueball DM horn:} \texttt{sims/glueball\_simp\_relic.py} →
  \texttt{GLUEBALL\_DM\_assessment.md}.
\item
  \textbf{External calculation specs (S-1/S-2/S-3):}
  \texttt{EXTERNAL\_CALC\_SPECS.md} (decision rules).
\item
  \textbf{\(\eta_B\) provenance / P3 meta-pattern:}
  \texttt{predict\_eta\_B.py}; \texttt{AUDIT\_round2\_sweep.md},
  \texttt{CLOSURES\_assumed\_vs\_calculated.md}.
\item
  \textbf{Pre-registration:}
  \texttt{PREREGISTRATION\_falsifier\_matrix.md} (Zenodo
  \texttt{10.5281/zenodo.21415326}).
\end{itemize}

Companion papers: {[}Paper III / SEDE cosmology, Zenodo DOI to be
assigned{]}, {[}Paper V / USC framework, Zenodo DOI to be assigned{]},
{[}Paper VI / scale sector, Zenodo DOI to be assigned{]}, {[}Paper VIII
/ APDM galactic, in preparation{]}.
